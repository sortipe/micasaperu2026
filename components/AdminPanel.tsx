...
const AdminPanel: React.FC<AdminPanelProps> = ({ user, properties, editingId, onAdd, onUpdate, onDelete, onCancel, showToast }) => {
  ...
  const uploadFile = async (file: File, folder: string): Promise<string | null> => {
    if (!isSupabaseConfigured) {
      showToast("Modo Demo: Supabase no configurado. Usando imagen local.", "WARNING");
  ...
  const handleSave = async () => {
    if (editingProperty && editingProperty.id) {
      ...
      const payload: any = {
        ...
        allowAdsUsage: cleanProperty.allowAdsUsage,
        lat: cleanProperty.lat,
        lng: cleanProperty.lng,
        agentId: (user.role === 'ADMINISTRADOR' && editingId) ? editingProperty.agentId : user.id,
      };
  ...
  const isModerating = user.role === 'ADMINISTRADOR' && editingProperty.agentId !== user.id;
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select className="w-full p-4 bg-gray-50 border-none rounded-2xl font-bold cursor-pointer" value={editingProperty.planType || 'BASIC'} onChange={(e) => updateField('planType', e.target.value)}>
                    ...
                    <option value="SUPER_FEATURED">Super Destacado</option>
                  </select>
                </div>
              </div>
                ...
                <div className="flex gap-6 items-center">
                  <label className="flex items-center gap-2 cursor-pointer group select-none">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${editingProperty.allowAdsUsage === true ? 'border-red-600' : 'border-gray-300 group-hover:border-red-400'}`}>
                      {editingProperty.allowAdsUsage === true && <div className="w-2 h-2 bg-red-600 rounded-full" />}
                    </div>
                    <input 
                      type="radio" 
                      name="allowAdsUsage" 
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group select-none">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${editingProperty.allowAdsUsage === false ? 'border-red-600' : 'border-gray-300 group-hover:border-red-400'}`}>
                      {editingProperty.allowAdsUsage === false && <div className="w-2 h-2 bg-red-600 rounded-full" />}
                    </div>
                    <input 
                      type="radio" 
                      name="allowAdsUsage" 
                      className="hidden"
                      checked={editingProperty.allowAdsUsage === false}
                      onChange={() => updateField('allowAdsUsage', false)}
                    />
                    <span className={`text-[9px] font-black uppercase tracking-wide ${editingProperty.allowAdsUsage === false ? 'text-slate-900' : 'text-gray-400'}`}>No, no permito</span>
                  </label>
                </div>
             </div>
