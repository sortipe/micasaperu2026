...
import { Property, User, Package, Transaction, PaymentMethod, LocationItem, Complaint, LegalDoc, LegalDocType, Notification, Inquiry, SocialLink, OfficeInfo } from './types';
...
const App: React.FC = () => {
  ...
  const [officeInfo, setOfficeInfo] = useState<OfficeInfo>({ address: 'Av. Benavides 768, Int. 1303, Miraflores, Lima', email: 'hola@aquivivir.com', phone: '' });
  ...
  const [view, setView] = useState<'HOME' | 'ADMIN' | 'DETAILS' | 'SEARCH' | 'PRICING' | 'DASHBOARD' | 'AUTH' | 'PAYMENT' | 'COMPLAINTS_BOOK'>('HOME');
  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        ...
        await Promise.all([fetchProperties(), fetchLocations(), fetchLegalDocs(), fetchPackages()]);
      ...
      } finally { setTimeout(() => setIsInitialLoading(false), 800); }
    };
    initApp();
  ...
  const fetchInquiries = async () => {
    ...
    if (currentUser.role !== 'ADMIN') query = query.eq('agentId', currentUser.id);
  ...
  const fetchProperties = async () => {
    try {
      const { data } = await supabase.from('properties').select('*, profiles:agentId(name, avatar, whatsapp)').order('createdAt', { ascending: false });
      if (data && data.length > 0) {
        const mapped = data.map((p: any) => ({ 
          ...p, 
          agentName: p.profiles?.name || 'Asesor', 
          agentAvatar: p.profiles?.avatar,
          agentWhatsapp: p.profiles?.whatsapp || p.agentWhatsapp
        }));
        setProperties(mapped as Property[]);
      } else { setProperties(INITIAL_PROPERTIES); }
    } catch (err) { setProperties(INITIAL_PROPERTIES); }
  };
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); setCurrentUser(null); setView('HOME'); handleClearFilters(); };
  ...
  const handleLogin = async (email: string, role: string, password?: string, isReg: boolean = false) => {
    ...
    if (isReg) {
      ...
      if (result.data.user) {
        const finalRole = email.toLowerCase() === ADMIN_EMAIL ? 'ADMIN' : (role || 'AGENT');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <Navbar user={currentUser} onNavigate={(v) => { setView(v); if(v === 'SEARCH') setSpatialFilterIds(null); if(v === 'HOME') handleClearFilters(); }} currentView={view} logo={appLogo} />
      <main className="flex-grow flex flex-col">
        {view === 'HOME' && (
          <>
            <SearchHero locations={locations} filters={filters} bannerUrl={homeBanner} onFilterChange={setFilters} onSearch={handlePerformSearch} />
            <DevelopmentOptions properties={properties} onPropertySelect={id => { setVisitedIds(p => new Set(p).add(id)); setSelectedPropertyId(id); setView('DETAILS'); }} currency={filters.currency} />
            <div className="container mx-auto px-4 py-16">
              {properties.filter(p => p.planType === 'SUPER_FEATURED').length > 0 && (
                <div className="mb-16">
                  <div className="flex items-center justify-between mb-10">
                    <h2 className="text-2xl font-black uppercase border-l-8 border-red-600 pl-6">Destacados Premium</h2>
                  ...
                  <PropertyList layout="slider" properties={properties.filter(p => p.planType === 'SUPER_FEATURED').slice(0, 6)} onPropertySelect={id => { setVisitedIds(p => new Set(p).add(id)); setSelectedPropertyId(id); setView('DETAILS'); }} currency={filters.currency} onClearFilters={handleClearFilters} visitedIds={visitedIds} />
                </div>
              )}
              ...
              {properties.filter(p => p.planType === 'FEATURED' || (p.isFeatured && p.planType !== 'SUPER_FEATURED')).length > 0 && (
                <div className="mb-16">
                  <div className="flex items-center justify-between mb-10">
                    <h2 className="text-2xl font-black uppercase border-l-8 border-blue-600 pl-6">Propiedades de Destacado</h2>
                    {properties.filter(p => p.planType === 'SUPER_FEATURED').length === 0 && (
                      <button onClick={() => handlePerformSearch('MAP')} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all flex items-center gap-2">
                         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                         Buscar por Mapa
                  ...
                  <PropertyList layout="slider" properties={properties.filter(p => p.planType === 'FEATURED' || (p.isFeatured && p.planType !== 'SUPER_FEATURED')).slice(0, 6)} onPropertySelect={id => { setVisitedIds(p => new Set(p).add(id)); setSelectedPropertyId(id); setView('DETAILS'); }} currency={filters.currency} onClearFilters={handleClearFilters} visitedIds={visitedIds} />
                </div>
              )}

              {properties.filter(p => p.planType === 'BASIC' || (!p.isFeatured && p.planType !== 'SUPER_FEATURED' && p.planType !== 'FEATURED')).length > 0 && (
                <div className="mb-16">
                  <div className="flex items-center justify-between mb-10">
                    <h2 className="text-2xl font-black uppercase border-l-8 border-slate-600 pl-6">Propiedades Normales</h2>
                    {properties.filter(p => p.planType === 'SUPER_FEATURED').length === 0 && properties.filter(p => p.planType === 'FEATURED' || (p.isFeatured && p.planType !== 'SUPER_FEATURED')).length === 0 && (
                      <button onClick={() => handlePerformSearch('MAP')} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all flex items-center gap-2">
                         <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                         Buscar por Mapa
                      </button>
                  ...
                  <PropertyList layout="slider" properties={properties.filter(p => p.planType === 'BASIC' || (!p.isFeatured && p.planType !== 'SUPER_FEATURED' && p.planType !== 'FEATURED')).slice(0, 6)} onPropertySelect={id => { setVisitedIds(p => new Set(p).add(id)); setSelectedPropertyId(id); setView('DETAILS'); }} currency={filters.currency} onClearFilters={handleClearFilters} visitedIds={visitedIds} />
                </div>
              )}
        ...
        {view === 'SEARCH' && (
          <div className="h-[calc(100vh-80px)] flex flex-col overflow-hidden">
            <div className="p-4 bg-white border-b shadow-sm z-[1100]">
               <SearchHero locations={locations} filters={filters} onFilterChange={setFilters} compact={true} activeLayout={searchLayout} onSearch={handlePerformSearch} />
            </div>
            ...
            <div className={`flex-grow overflow-y-auto p-6 custom-scrollbar ${searchLayout === 'LIST' ? 'block' : 'hidden'}`}>
              <div className="max-w-7xl mx-auto pb-6">
                 <PropertyList layout="list" properties={filteredProperties} onPropertySelect={id => { setSelectedPropertyId(id); setView('DETAILS'); }} currency={filters.currency} onClearFilters={handleClearFilters} visitedIds={visitedIds} />
              </div>
            </div>
            ...
            <div className={`flex-grow relative ${searchLayout === 'MAP' ? 'block' : 'hidden'}`}>
              <MapView 
                ...
                onSelectProperty={id => { setSelectedPropertyId(id); setView('DETAILS'); }} 
        ...
        {view === 'DASHBOARD' && currentUser && (
          <ClientDashboard 
            ...
            appLogo={appLogo} homeBanner={homeBanner} socialLinks={socialLinks} officeInfo={officeInfo}
            ...
            onLogout={handleLogout} onUpdateTransactionStatus={async (id, s) => { await supabase.from('transactions').update({status: s}).eq('id', id); fetchTransactions(); showToast("Estado de transacción actualizado", "SUCCESS"); }} onSaveLegalDoc={async d => { await supabase.from('legal_documents').upsert(d); fetchLegalDocs(); showToast("Documento legal guardado", "SUCCESS"); }} 
            onSavePackage={async p => { 
              const { error } = await supabase.from('packages').upsert(p); 
              if (error) {
                if (error.code === '42P01') {
                  showToast("Falta crear la tabla 'packages' en Supabase", "ERROR");
                } else if (error.code === 'PGRST204') {
                  const { superFeaturedLimit, ...fallbackP } = p;
                  await supabase.from('packages').upsert(fallbackP);
                  fetchPackages();
                  showToast("Plan guardado (sin Súper Destacados, falta columna)", "WARNING");
                } else {
        ...
        {view === 'ADMIN' && (
          <AdminPanel 
            ...
            onAdd={async p => { 
              ...
              const { error } = await supabase.from('properties').insert(cleanP); 
              if (error) {
        ...
        {view === 'DETAILS' && selectedPropertyId && (
          properties.some(p => p.id === selectedPropertyId) ? (
            <PropertyDetails property={properties.find(p => p.id === selectedPropertyId)!} onBack={() => setView('SEARCH')} onSendMessage={handleSendMessage} showToast={showToast} />
          ) : (
        ...
        {view === 'PRICING' && <PricingPage onSelectPackage={p => { setSelectedPackage(p); if (currentUser) setView('PAYMENT'); else setView('AUTH'); }} />}
        {view === 'PAYMENT' && selectedPackage && currentUser && (
          <PaymentFlow
            pkg={selectedPackage}
            user={currentUser}
        ...
        {view === 'AUTH' && <AuthPage onLogin={handleLogin} onBack={() => { setView('HOME'); handleClearFilters(); }} />}
        {view === 'COMPLAINTS_BOOK' && <ComplaintsBook onSave={async c => { await supabase.from('complaints').insert(c); }} onClose={() => { setView('HOME'); handleClearFilters(); }} showToast={showToast} />}
      </main>
