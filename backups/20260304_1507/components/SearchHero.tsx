...
interface SearchHeroProps {
  ...
  onSearch: (layout?: 'LIST' | 'MAP', focusLoc?: string, overrideFilters?: any) => void;
  compact?: boolean;
...
const SearchHero: React.FC<SearchHeroProps> = ({
  ...
  const getFullLocationName = (loc: LocationItem) => {
    let parts = [loc.name];
    if (loc.parent) {
      parts.push(loc.parent);
      parts.push(loc.parent); // Agregamos el padre dos veces para simular Provincia y Departamento
    }
    return parts.join(', ');
  };

  const filteredSuggestions = locations.filter(loc => {
    const searchStr = normalizeText(getFullLocationName(loc));
    const queryStr = normalizeText(filters.query);
    return filters.query.length > 0 && searchStr.includes(queryStr);
  }).slice(0, 10);
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full">
          ...
          <div className="grid grid-cols-2 sm:flex gap-2">
            ...
            <button 
              ...
              className="col-span-2 sm:col-span-1 h-[50px] px-10 rounded-xl font-black text-[11px] uppercase tracking-widest bg-red-600 text-white hover:bg-slate-900 shadow-xl active:scale-95 transition-all"
            >
              BUSCAR
            </button>
          </div>
            ...
            <div className="bg-white rounded-b-[1.5rem] rounded-tr-[1.5rem] p-2 md:p-3 shadow-2xl relative overflow-visible border border-white">
              <div className="flex flex-col md:flex-row items-center w-full gap-2">
                ...
                <button 
                  ...
                  className="bg-red-600 hover:bg-slate-900 text-white px-6 md:px-12 h-10 md:h-14 rounded-xl md:rounded-[1.2rem] font-black text-xs md:text-sm uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95 w-full md:w-auto"
                >
                  BUSCAR
                </button>
              </div>
